...
Chat.prototype.checkLostMessages = function (id) {
  ...
  receivers.forEach(function (name) {
    ...
    if (receiver.received) {
      received++;
      if (name.split(":")[0] === data.origin.split(":")[0]) {
        // If same IP
        receivedFromSelf++;
      }
    }
  ...
  var percentReceived = parseFloat(
      ((received / receivers.length) * 100).toFixed(2)
    ),
  ...
  if (
    percentLost === 100 ||
    (receivers.length > 3 && received === receivedFromSelf)
  ) {
  ...
  if (percentLost > 0) {
    this.storedLostMessages.push({
      origin: {
        server: data.origin,
        ip: data.origin.split(":")[0],
        port: parseInt(data.origin.split(":")[1]),
      },
...
Chat.prototype.setup = function (server) {
  ...
  server.client.on("disconnected", function () {
    console.log(
      "%s disconnected on port %d over %s",
      server.host,
      server.port,
      server.protocol
    );
  ...
  server.clientMonitor.on("connected", function () {
    console.log(
      "%s (monitor) connected on port %d over %s",
      server.host,
      server.port,
      server.protocol
    );
  ...
  server.clientMonitor.on("privmsg", function (data) {
    ...
    if (server.name === params[0]) {
      ...
      if (server.pings.length <= 6) {
        ...
        for (
          var i = server.pings.length - 1;
          i >= server.pings.length - 7;
          i--
        ) {
